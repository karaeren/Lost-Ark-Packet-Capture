...
namespace Lost_Ark_Packet_Capture
{
    ...
    public class TcpReconstruction
    {
        ...
        private void ReassembleTcp(ulong sequence, ulong length, byte[] data, ulong data_length, bool synflag, long net_src, long net_dst, uint srcport, uint dstport, PacketDotNet.TcpPacket tcpPacket)
        {
            ...
            for (j = 0; j < 2; j++) if (src_addr[j] == srcx && src_port[j] == srcport) src_index = j;
            ...
            if (sequence == seq[src_index])
            {
                ...
                if (data != null) write_packet_data(src_index, data, tcpPacket);
        ...
        bool check_fragments(int index, PacketDotNet.TcpPacket tcpPacket)
        {
            ...
            while (current != null)
            {
                if (current.seq == seq[index])
                {
                    ...
                    if (current.data != null) write_packet_data(index, current.data, tcpPacket);
